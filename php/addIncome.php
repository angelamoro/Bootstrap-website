...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $sql = "SELECT id_category FROM tracker.categories WHERE name=':category'";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':category', $category);

        $idCategory = $stmt->execute();


        try {

            $sql2 = "insert into tracker.transactions (type, id_category, amount, date, description, id_user) 
                values ('Income', :category, :amount, ':date', ':description', :id_user)";
            $stmt1 = $pdo->prepare($sql2);
            $stmt1->bindParam(':category', $idCategory);
